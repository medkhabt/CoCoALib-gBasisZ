...
namespace CoCoA
{
    const long NEGATIVE = -2147483647L; // OK on all binary computers
    const long POSITIVE =  2147483647L; // OK on all binary computers
  ...
  // [this is both good and bad].
    ...
    long addition_overflowSafe(long* num1, long* num2) {
        ...
        if(MOST_POSITIVE - *num1 < *num2) {
            throw std::out_of_range("Overflow when trying to add num1 to num2 ");
        }
        else {
            return *num1 + *num2 ;
        }
      ...
      long addition_vector_overflowSafe(const std::vector<long> &longVector){
          long result = 0 ;
          for ( auto  element : longVector ) {
              result = addition_overflowSafe(&result, &element);
          }
          
          return result;
      }
    
    long substraction_overflowSafe(long* num1, long* num2) {
        if( num2 + MOST_NEGATIVE > num1 ) {
            throw std::out_of_range("Overflow when trying to substract num1 to num2");
        }
        else {
            return *num1 - *num2;
        }
    ...
    long substraction_vector_overflowSafe(const std::vector<long> &longVector){
        long result = 0;
        for( auto element: longVector ) {
            result = substraction_overflowSafe(&result, &element);
        }
        return result;
    }
    
    long multiplication_overflowSafe(long* num1, long* num2) {
        if( num1 == 0L || num2 == 0L) {
            return 0L;
        }
        else if( *num1 > std::ceil((long)MOST_POSITIVE / *num2) ) {
            throw std::out_of_range("Overflow whe trying to multiplate num1 to num2");
        }
        else {
            return *num1 * *num2;
        }
    ...
    long multiplication_vector_overflowSafe(const std::vector<long> &longVector) {
        long result = 1L;
        for ( auto element: longVector ) {
            result = multiplication_overflowSafe(&result, &element);
        }
        return result;
    }
    
    long division_vector_overflowSafe(const std::vector<long> &longVector) {
        // if we are looking also for negative values than we need to check -1 else just 0.
...
//    cout << "Trying the addition function for a vector" << endl;
      std::vector<long> v = { 10, 14, 5, 1000000L, 300000L};
//
//      cout << " result is : addition is " << addition_vector_overflowSafe(v) ;
//
//      cout << "Trying the substraction function for a vector " << endl;
//      cout << " result is : subb is " << substraction_vector_overflowSafe(v);
      
      
      cout << "Trying to multiplate function for a vector " << endl;
      cout << "Result is : mult is " << multiplication_vector_overflowSafe(v) << endl ;
      
      cout << "Trying to divide min with -1 : " << MOST_NEGATIVE / -1 << endl;
